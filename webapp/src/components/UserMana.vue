  ...
  export default{
    ...
    methods: {
      saveRoles(id, index){
        ...
        httpPut("/admin/user/role", {rids: this.roles, id: id}).then(resp=> {
      ...
      deleteUser(id){
        ...
        this.$confirm('删除该用户, 是否继续?', '提示', {
          ...
          _this.loading = true;
          httpDelete("/admin/user/" + id).then(resp=> {
            if (resp.status == 200 && resp.data.status == 'success') {
      ...
      enabledChange(enabled, id, index){
        var _this = this;
        _this.cardloading.splice(index, 1, true)
        httpPut("/admin/user/enabled", {enabled: enabled, uid: id}).then(resp=> {
          if (resp.status != 200) {
            _this.$message({type: 'error', message: '更新失败!'})
            _this.loadOneUserById(id, index);
            return;
      ...
      loadOneUserById(id, index){
        var _this = this;
        httpGet("/admin/user/" + id).then(resp=> {
          _this.cardloading.splice(index, 1, false)
          if (resp.status == 200) {
      ...
      loadUserList(){
        var _this = this;
        httpGet("/admin/user?nickname="+this.keywords).then(resp=> {
          _this.loading = false;
          if (resp.status == 200) {
