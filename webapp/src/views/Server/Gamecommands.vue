<template>
  <div>
    ...
    <el-checkbox-group v-model="checkedServers" @change="handleCheckedChange">
      <el-checkbox v-for="server in servers" :label="server" :key="server">{{ server.name }}</el-checkbox>
    </el-checkbox-group>


    <el-table ref="multipleTable" :data="commands" tooltip-effect="dark" style="overflow-x: hidden; overflow-y: hidden;"
      v-loading="loading">
      <el-table-column label="后台命令" width="200" align="left">
        <template slot-scope="scope"><span style="color: #409eff;cursor: pointer" @click="itemClick(scope.row)">{{
          scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="参数说明" width="400" align="left">
        <template slot-scope="scope">{{ scope.row.params }}</template>
      </el-table-column>
      <el-table-column label="参数" width="400" align="left">
...
export default {
  ...
  methods: {
    ...
    loadCommands() {
      this.$api.server.findCommands({ type: 0 }).then((res) => {
        this.commands = res;
      })
    },
    ...
    submitClick() {
      ...
      this.$api.server.execCommand({
        selectedServers: this.checkedServers.map(node => { return node.id }).join(";"),
        type: this.cmdType,
